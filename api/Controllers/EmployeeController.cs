...
namespace api.Controllers
{
    ...
    public class EmployeeController : ControllerBase
    {
        ...
        public async Task<ActionResult> AddEmployee([FromBody] EmployeeUpdateAndCreateDto employeeDto, int id)
        {
            if (employeeDto == null)
            {
                return BadRequest("Dữ liệu không hợp lệ, vui lòng kiểm tra lại");
            }
            var checkExist = await _employeeRepo.GetEmployeeByIdAsync(id);
            if (checkExist != null)
            {
                return BadRequest("Nhân viên đã tồn tại, vui lòng kiểm tra lại");
            }
            var created = await _employeeRepo.AddAsync(employeeDto, id);
            return CreatedAtAction(nameof(GetEmployeeById), new { id = created.EmployeeID }, created);
        }

        [HttpPut("update/{id}")]
        public async Task<ActionResult> UpdateEmployee([FromBody] EmployeeUpdateAndCreateDto employeeDto, int id)
        {
            if (employeeDto == null)
            {
